...
#Used to rescale image intensities to match initial images
initialAvg = np.nanmean(differenceImages[0])
initialStd = np.nanstd(differenceImages[0])

for image in differenceImages:

    #Current average and standard deviation of pixel values
    avg = np.nanmean(image)
    std = np.nanstd(image)

    #Force same mean and standard deviation as intial image
    newImage = initialAvg + ((image - avg) * (initialStd / std))

    #force same range as before - shouldn't have changed but just in case outliers
    np.clip(newImage, -1, 1)
    
    #Adjusting to 0 -> 255 range instead of -1 -> 1
    image = newImage
    image = (image + 1)/2
    image = image * 255

    image = np.array(image, dtype="uint8")  #Datatype needed by openCV

    #To reduce dead pixels
...
#Normalising image intensites as before

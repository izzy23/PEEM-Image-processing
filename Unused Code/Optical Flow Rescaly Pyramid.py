...
def rotateImg(img, da):
    ...
    #Finds centre of rotation - centre of image
    h, w = img.shape
    center = (w / 2, h / 2)

    scale = 1
    #gets rotation matrix with no size change
    m = cv2.getRotationMatrix2D(center, da, scale)

    #Applies matrix to rotate image
...
def align_images_tps(image1, image2, num_scales=10):
    ...
    # Convert images to the required data type (8-bit unsigned integer) - for openCV
    image1 = (image1 * 255).astype(np.uint8)
    image2 = (image2 * 255).astype(np.uint8)

    image_shape = image1.shape[::-1]  # Reverse the shape to (height, width)

    #Builds pyramid of resized images - stacks shrunk by constant scale factor
    ...
    for level in reversed(range(num_scales)):
        ...
        # Alternative curve smoothing to other program
        ...
        # If points are found, set true
        feature_points_found = True
        break  # Successful alignment :)

...
differenceImages = np.array(differenceImages, order="c")

#maskImage = sitk.OtsuThreshold(differenceImages[0], 0, 1, 200)

#maskImage = sitk.OtsuThreshold(differenceImages[0])
corrector = sitk.N4BiasFieldCorrectionImageFilter()
numberFittingLevels = 4

#if len(differenceImages) > 6:
#    numberFittingLevels = int(differenceImages[6])
#if len(differenceImages) > 5:
#    corrector.SetMaximumNumberOfIterations(
#            [int(differenceImages[5])] * numberFittingLevels
#    )
...
for i in range(0, len(differenceImages)):
    ...
    correctedImg = corrector.Execute(croppedImage, maskImage)
    log_bias_field = corrector.GetLogBiasFieldAsImage(croppedImage)
    correctedImage = croppedImage / sitk.Exp(log_bias_field)
    ...
    #p2, p98 = np.percentile(croppedImage, (0.1, 99.9))
    p2, p98 = np.percentile(croppedImage, (1, 99))
    print("p2 = " + str(p2))
    print("p98 = " + str(p98))


    #croppedImage = rescale_intensity(croppedImage, in_range=(p2, p98))

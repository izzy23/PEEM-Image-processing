...
differenceImages = [io.imread(r"finalCross/alignedCrossNewFinalFinal_D_0.tif")]
intensityImages = [io.imread(r"finalCross/alignedCrossNewFinalFinal_I_0.tif")]

for i in imageNumbers: 

    differenceImg = io.imread(r"finalCross/alignedCrossNewFinalFinal_D_%s.tif" % str(i))
    intensityImg = io.imread(r"finalCross/alignedCrossNewFinalFinal_I_%s.tif" % str(i))
    print("shape")
...
for i in range(0, len(differenceImages)):
    ...
    for x in range(0, 3):
        ...
        oldAngles = np.array([0, 0, -28, -45, -50, -50, 0])
    ...
    def buttonFunc(label):
        ...
        if label == "+ rotate":
            m = cv2.getRotationMatrix2D([imgH/2, imgW / 2], 0.25, 1)
            newImg = cv2.warpAffine(differenceImages[i], m, (w,h), flags = cv2.INTER_CUBIC)
            differenceImages[i] = newImg
            angles[i] = angles[i] + 0.25
        if label == "- rotate":
            m = cv2.getRotationMatrix2D([imgH/2, imgW / 2], -0.25, 1)
            newImg = cv2.warpAffine(differenceImages[i], m, (w,h), flags = cv2.INTER_CUBIC)
            differenceImages[i] = newImg
...
for i in range(0, len(differenceImages)):
    sk_imsave("finalCross/alignedCrossNewFinalFinal1_D_%s.tif" %str(i), differenceImages[i])
    sk_imsave("finalCross/alignedCrossNewFinalFinal1_I_%s.tif" %str(i), intensityImages[i])

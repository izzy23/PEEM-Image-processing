...
correctedDifferenceImages = correctedDifferenceImages + [initialDifferenceImages[0]]
correctedIntensityImages = correctedIntensityImages + [initialIntensityImages[0]]
for i in range(0, len(differenceImages)):
    ...
    displayImg = differenceImages[i]
    plt.imshow(displayImg, cmap="gray")
    plt.show()
    displayImg = np.clip(displayImg, -0.08, 0.1)
    displayImg = (displayImg-np.nanmin(displayImg))/(np.nanmax(displayImg)-np.nanmin(displayImg))
    displayImg = displayImg * 255
    displayImg = np.round(displayImg)
    displayImg= np.array(displayImg, dtype = "uint8")
    plt.imshow(displayImg, cmap="gray")
    plt.show()
    ...
    for x in range(0, 8):
        ...
        print("showing image before selection")
        plt.imshow(displayImg, cmap="gray")
        plt.show()
...
for i in range(0, len(correctedDifferenceImages)):
    sk_imsave("finalCross/alignedCrossFinal6_D_%s.tif" % str(i), correctedDifferenceImages[i])
    sk_imsave("finalCross/alignedCrossFinal6_I_%s.tif" % str(i), correctedIntensityImages[i])
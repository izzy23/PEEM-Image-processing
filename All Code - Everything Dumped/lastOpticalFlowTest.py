...
def driftCorrect(imgArr):
    ...
    firstImg = np.array(imgArr[0], dtype="uint8")
    print("showing fist img")
    ...
    for i in range(0, len(imgArr)-1):
        ...
        m = cv2.estimateAffinePartial2D(curr_pts, prev_pts)
        #print("m")
        #print(m)
...
for i in imageNumbers: 
    ...
    differenceImg = io.imread(r"finalCross/alignedCrossNewCentreTest25_D_%s.tif" % str(i))
    intensityImg = io.imread(r"finalCross/alignedCrossNewCentreTest25_I_%s.tif" % str(i))
...
alignedImages = driftCorrect(differenceImages)

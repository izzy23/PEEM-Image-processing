...
zooms = np.ones(len(differenceImages))
for i in range(1, len(differenceImages)):
    ...
    fig1 = ax1.imshow(differenceImages[0], cmap="gray")
    img2 = cv2.resize(differenceImages[i], (0, 0), fx = 1, fy = 1, interpolation = cv2.INTER_LINEAR)
    ...
    def update(val):
        ...
        if sb.val >= 1:
            ...
            zoom = 1- sb.val
        img2 = cv2.resize(differenceImages[i], (0, 0), fx = zoom, fy = zoom, interpolation = cv2.INTER_LINEAR)
        w, h = img2.shape
        initialW, initialH = differenceImages[i].shape
...
for i in range(0, len(differenceImages) - 1):
    currentZoom = zooms[i]
    print("current zoom = " + str(currentZoom))
    img = cv2.resize(differenceImages[i+1], (0, 0), fx = float(currentZoom), fy = float(currentZoom), interpolation = cv2.INTER_LINEAR)
    w, h = img.shape

...
def driftCorrect(imgArr):
    ...
    firstImg = imgArr[0]
    firstImg = (firstImg-np.nanmin(firstImg))/(np.nanmax(firstImg)-np.nanmin(firstImg)) #renormalises - after contrast adjust so shows
    firstImg = firstImg * 255
    firstImg = np.clip(firstImg, 0, 255)
    ...
    for i in range(0, len(imgArr)-1):
        ...
        if i+1 < len(imgArr):
            current = imgArr[i+1]
            current = (current-np.nanmin(current))/(np.nanmax(current)-np.nanmin(current)) #renormalises - after contrast adjust so shows
            #current = current * 255
            #current = np.clip(current, 10, 70)
            #current = (current-np.nanmin(current))/(np.nanmax(current)-np.nanmin(current)) #renormalises - after contrast adjust so shows
            current = current * 255
        ...
        smoothedImage = cv2.warpAffine(current, m, (w,h))
        preSmoothedImages = preSmoothedImages + [smoothedImage]
...
#differenceImages = [io.imread(r"finalCross/alignedCross10_D_0.tif")]
#intensityImages = [io.imread(r"finalCross/alignedCross10_I_0.tif")]
differenceImages = [io.imread(r"finalCross/alignedCrossNewCentreTest7_D_0.tif")]
intensityImages = [io.imread(r"finalCross/alignedCrossNewCentreTest7_I_0.tif")]

for i in imageNumbers: 

    differenceImg = io.imread(r"finalCross/alignedCrossNewCentreTest7_D_%s.tif" % str(i))
    intensityImg = io.imread(r"finalCross/alignedCrossNewCentreTest7_I_%s.tif" % str(i))
    print("shape")
...
correctedEntireStack = driftCorrect(differenceImages)
...
average = averageImages(correctedEntireStack)

fig1 = ax1.imshow(preSmoothedImages[0], cmap="gray")
